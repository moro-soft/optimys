...
class Roll {// рулон (единица груза) для загрузки в ТС или состав ТС (Контейнерный поезд)
	constructor(id, H, D, W) {
		...
		this.pId = 0;
		this.placementError = '';
		this.color = wa.styleColorFillRoll(this);
	}
}// конец класса
...
class Container { // Траспортное стредство для погрузки
	...
	getCapacityClass(container) { // получить класс состояния
    ...
    finalizePlacement() {
        ...
        this.rolls.forEach(roll => {
            wa.originalRollPositions.set(roll.id, {
                x: roll.x,
                y: roll.y,
	...
	existSomeCross(arrPos,pos){ // есть хотя бы с одним пересечение
		return arrPos.some(p => DivCentrRoll(p,pos) > wa.mm);
	}
	existSomeCrossRoll(arr,pos){ // есть хотя бы с одним пересечение
		return arr.some(p => p.s === pos.s && DivCentrRoll(p,pos) > wa.mm); // Dist(p,pos) > mm
	}
   ...
   // метод предрасчета позиций начиная с параметра
    precalculatePositionsRolls(startX = 0) {
        ...
        while (this.current_X <= (this.gateTypeCenter ?  this.L/2 - this.T/4 :  this.L - this.oRolls.R - this.minCMDoor) ) {// Разделяем расчет для разных типов ТС
			...
			if (this.rowNumber === 0) {	// Первый ряд - базое заполнение
			...
			if (validPositions.length > 0) {
				this.current_X = Math.min(...validPositions.map(p => p.x)) + wa.mm; 
			} else {
	...
	generateFirstRow(start_Y, direction) {
		...
		for (let y = ( for2roll && random < 0.1 ? this.T/2 : start_Y ) ; 
			...
			positions.push({
				type: 'first',
				priority: 1,
				x: this.current_X, // Math.max(this.current_X, this.minDimX + this.oRolls.R), // Добавлена проверка упора в стен
				y: y,
				D: this.oRolls.D,
				R: this.oRolls.R,
				s: 0
	...
	calculateBetweenPositions(prevPos, direction) {
		...
		if (prevPos.length > 1 ) { 
			for (let i = 0; i < prevPos.length - 1; i++) {
				...
				if (center && Math.abs(center.x-this.current_X) < this.oRolls.R*0.866 + wa.mm) {
					positions.push({
						type: 'между',
	...
	calculatePositions(prevPos, start_Y, direction) {
		...
		for (let y = start_Y; 
			...
			if (y === start_Y) {
				...
				const cur_prevPos = prevPos.filter(p => Math.abs(p.y - y) < (p.R + this.oRolls.R) + wa.mm);//console.log(cur_prevPos)
	...
	exist2Contact(pos) {
		...
		if (pos.x - pos.R <= wa.mm || pos.x + pos.R >= this.L - (this.gateTypeCenter ? 0 : this.minCMDoor) - wa.mm ) { 
			if (pos.y === this.T/2) { //крайний по центру допускается 
				return true;
			}	
			contactPoints++;				
		}	
		if (pos.y - pos.R <= wa.mm || pos.y + pos.R >= this.T - wa.mm) contactPoints++;
		let xmin = pos.x - pos.R - wa.mm;	
		let xmax = pos.x + pos.R + wa.mm;	
		...
		for (const p of posRoll ) { //рулонов около по ширине
			if (Math.abs( DivCentrRoll(p, pos) ) <= wa.mm) { // точное касание
				contactPoints++; 	
	...
	compactRollsInRange(startX, endX) {
		...
		Object.values(positionGroups).forEach(rollsInPos => {
			...
			const bottomNeighbors = this.rolls.filter(r => 
				r.y < baseRoll.y && 
				Math.abs(r.x - baseRoll.x) < r.R + baseRoll.R + wa.mm
			);
			...
			if (newY - baseRoll.R <= wa.mm) contacts++; // Касание нижней стенки
			...
			const leftNeighbors = this.rolls.filter(r => 
				r.x < baseRoll.x && 
				Math.abs(r.y - newY) < r.R + baseRoll.R + wa.mm
			);
			...
			if (newX - baseRoll.R <= wa.mm) contacts++; // Касание левой стенки
			if (newY - baseRoll.R <= wa.mm || newY + baseRoll.R >= this.T - wa.mm) contacts++; // Касание стенок по Y
	...
	addValidPositions(Positions, check2Contact = true ) {
		for (const pos of Positions) {
			if ( !check2Contact || this.exist2Contact(pos) ) {
				if ( ! this.existSomeCross( this.Positions.filter(p =>(p.x >= pos.x - (this.oRolls.R + pos.R) - wa.mm ) 
																	&&(p.x <= pos.x + (this.oRolls.R + pos.R) + wa.mm )
																)
											, pos )) {
					for (let s = 1; s <= 3; s++) {
						this.Positions.push({	
	...
	isPositionOccupied(x, y, s) {
		return this.rolls.some(roll => 
			Math.abs(roll.x - x) < wa.mm && 
			Math.abs(roll.y - y) < wa.mm &&
			roll.s === s
    );
}	
	...
	swapRollCenter() {	
		...
		if (Math.abs(this.rightW - this.leftW) > this.maxKGT) {	
			...
			const rolls = topSRols.filter(r => r.id!==targetRoll.id && Math.abs(r.x-targetRoll.x)-wa.mm < r.R+targetRoll.R).sort( (a,b) => kDoor*(b.x - a.x) );//console.log(rolls)	
	...
	findTwoClosestRolls(roll) {
		...
		for (let i = 0; i < candidates.length; i++) {
			for (let j = i + 1; j < candidates.length; j++) {
				...
				const minAllowedDist = (roll1.R + roll2.R - wa.mm);
				console.log(minAllowedDist)
				const maxAllowedDist = (roll1.R + roll.D + roll2.R + wa.mm);//	console.log(maxAllowedDist)
    ...
    snapRollToContacts(roll) {
        ...
        for (const pos of newPositions) {
            ...
            for (const otherRoll of this.rolls) {
                ...
                if (DistCentrRoll(otherRoll, {x: pos.x, y: pos.y}) < (otherRoll.D + roll.D)/2 - wa.mm) {
                    hasCollision = true;
                    break;
                }
            }
...
class Train { // Состав (цепочка) ТС
	...
	addContainer(params) {
        ...
        // Обновляем навигацию
        if (wa.oContainerIndex === this.containers.length - 2) {
            wa.oContainerIndex++;
        }
        wa.showContainer();
    }
 ...
 */ 
	...
	async optimizeLoading(rolls) {
		...
		if (this.unplacedRolls.length > 0) {
			...
			this.unplacedRolls = result ? result.unplaced || [] : [];
...
function calculateTotalHeightAtPosition(x, y, s) {
    let totalHeight = 0;
    for (const roll of wa.oContainer.rolls) {
		if (Math.abs(roll.x - x) < wa.mm && Math.abs(roll.y - y) < wa.mm && roll.s < s) {
			totalHeight += roll.H;
		}
    }
    return totalHeight;
...
async function handleExcelUploadV1(event) {
    ...
    try {
		...
		if (rollSetsContainer) {
			...
			wa.rollSets.forEach((roll, index) => {
...
console.log("Структура файла:", {
	...
	Sheets: Object.keys(workbook.Sheets)
        ...
        //const wa.rollSets = generateRollSets(rolls);
		// Обновляем UI с новыми rollset (если нужно)
		if (wa.rollSets && rolls && rolls.length > 0) {
...
function updateRollSetsUI(_rollSets) {
    const rollSetsContainer = document.getElementById('roll-sets-container');
...
function initExcelImport() {
    ...
    fileInput.addEventListener('change', async (event) => {
        ...
        try {
            ...
            if (rolls && rolls.length > 0) {
                ...
                wa.showMessage(`!!!Успешно импортировано ${rolls.length} рулонов`);
            }
        } finally {
            hideLoadingProgress();
	...
	wa.controls = document.getElementById('controls-container');
    if (wa.controls) {
        wa.controls.appendChild(importBtn);
    }
}
...
function saveOriginalPositions() { //сохранение исходных позиций
    wa.originalRollPositions.clear();
    wa.currentTrain.containers.forEach(container => {
...
function getSchemeData() {// Получить данные схемы
    if (!wa.currentTrain || wa.currentTrain.containers.length === 0) return null;
    ...
    const containersData = wa.currentTrain.containers.map(container => ({
        params: {
            L: container.L,
            T: container.T,
            H: container.H,
    ...
    return {
        ...
        trainParams: {
            containerCount: wa.currentTrain.containers.length,
            totalWeight: wa.currentTrain.totalWeight()
        },
        containers: containersData,
        rollSets: wa.rollSets,
        unplacedRolls: wa.currentTrain.unplacedRolls.map(r => ({
            id: r.id,
            H: r.H,
            D: r.D,
            W: r.W,
        ...
        uiState: {
            ...
            currentContainerIndex: wa.oContainerIndex
        }
    };
}
    ...
    if (scheme.version === "1.1" && scheme.containers) {
        ...
        scheme.containers.forEach(containerData => {
            const container = new Container({
                ...
                trainIndex: wa.currentTrain.containers.length,
                id: `container-${Date.now()}-${wa.currentTrain.containers.length}`
            });
            ...
            container.rolls = containerData.rolls.map(r => {
                ...
                Object.assign(roll, {
                    ...
                    s: r.s,
                    row: r.row,
                    color: r.color || wa.styleColorFillRoll(roll)
                });
                return roll;
            });
            ...
            wa.currentTrain.containers.push(container);
        });
        
        // Восстанавливаем неразмещенные рулоны
        if (scheme.unplacedRolls) {
            wa.currentTrain.unplacedRolls = scheme.unplacedRolls.map(r => {
                const roll = new Roll(r.id, r.H, r.D, r.W);
        ...
        wa.oContainerIndex = scheme.uiState?.currentContainerIndex || 0;
        if (wa.oContainerIndex >= wa.currentTrain.containers.length) {
            wa.oContainerIndex = wa.currentTrain.containers.length - 1;
        }
        wa.oContainer = wa.currentTrain.containers[wa.oContainerIndex];
    } else {
